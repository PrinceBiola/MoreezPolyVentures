  ...
  const handleDownloadReport = (reportType) => {
    ...
    switch (reportType) {
      ...
      case 'driver-sales': {
        ...
        exportToCSV(reportData, 'Driver_Sales_Report');
        break;
      }
      case 'inventory': {
    ...
    <div className="space-y-6 animate-fade-in text-left pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
        <div className="flex items-center gap-6">
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase leading-none">Reports</h2>
        </div>
        <div className="flex gap-3">
          <div className="relative">
             <button 
                onClick={() => setShowPeriodDropdown(!showPeriodDropdown)}
                className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-md text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all"
             >
                {period} <ChevronDown className={`w-3 h-3 opacity-40 transition-transform ${showPeriodDropdown ? 'rotate-180' : ''}`} />
             </button>
             {showPeriodDropdown && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-md shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                   {['This Month', 'Last Month', 'This Year', 'All Time'].map(p => (
                      <button 
                         key={p}
                         onClick={() => { setPeriod(p); setShowPeriodDropdown(false); }}
                         className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 border-b border-slate-50 last:border-0"
                      >
                         {p}
                      </button>
                   ))}
          ...
          <div className="flex relative">
             <button 
                ...
                className="flex items-center gap-2 px-6 py-2 bg-emerald-600 text-white rounded-l-md text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all border-r border-emerald-500/30"
             >
                Download Report
             </button>
             <button 
                onClick={() => setShowReportDropdown(!showReportDropdown)}
                className="px-3 bg-emerald-700 text-white rounded-r-md hover:bg-emerald-800 transition-all"
             >
                <ChevronDown className={`w-4 h-4 transition-transform ${showReportDropdown ? 'rotate-180' : ''}`} />
             </button>

             {showReportDropdown && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-slate-200 rounded-md shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                   {[
                      { label: 'Expenses Report', type: 'expenses' },
                      { label: 'Driver Sales', type: 'driver-sales' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           ...
           <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Area type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorRev)" dot={{ fill: '#10b981', r: 4, strokeWidth: 2, stroke: '#fff' }} />
                 </AreaChart>
              </ResponsiveContainer>
           </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-md p-8 shadow-sm">
           <h3 className="text-sm font-black text-slate-900 tracking-tighter uppercase mb-8">Expense Distribution</h3>
           <div className="h-[250px] relative">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Operational</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
         ...
         <div className="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
               <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Business Summary</h3>
            </div>
            <table className="w-full text-left">
               <thead>
                  <tr className="border-b border-slate-100">
                     ...
                     <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Revenue (Est)</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-50 text-[11px] font-bold">
                  {data.business.slice(0, 4).map((p, i) => (
                     <tr key={i}>
                        <td className="px-6 py-4 text-slate-900 uppercase">{p.name}</td>
                        <td className="px-6 py-4 text-slate-400">{p.currentStock} Units</td>
                        <td className="px-6 py-4 text-right text-slate-900 tabular-nums">₦{(p.sellingPrice * p.currentStock || 0).toLocaleString()}</td>
                     </tr>
                  ))}
                  <tr className="bg-slate-50/50 font-black text-[12px]">
                     <td className="px-6 py-4 uppercase">Totals</td>
                     <td className="px-6 py-4 text-slate-400">{businessTotalUnits} Units</td>
                     <td className="px-6 py-4 text-right tabular-nums">₦{businessTotalRevenue.toLocaleString()}</td>
                  </tr>
               </tbody>
            </table>
         </div>
         ...
         <div className="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
               <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Transport Summary</h3>
            </div>
            <table className="w-full text-left">
               <thead>
                  <tr className="border-b border-slate-100">
                     <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Vehicle</th>
                     <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Trips</th>
                     <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Income</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-50 text-[11px] font-bold">
                  {data.transport.slice(0, 4).map((p, i) => (
                     <tr key={i}>
                        <td className="px-6 py-4 text-slate-900 uppercase">{p.carId?.plateNumber || 'FLEET-NODE'}</td>
                        <td className="px-6 py-4 text-slate-400">Record</td>
                        <td className="px-6 py-4 text-right text-slate-900 tabular-nums">₦{(p.amount || 0).toLocaleString()}</td>
                     </tr>
                  ))}
                  <tr className="bg-slate-50/50 font-black text-[12px]">
                     <td className="px-6 py-4 uppercase">Totals</td>
                     <td className="px-6 py-4 text-slate-400">{data.transport.length} Records</td>
                     <td className="px-6 py-4 text-right tabular-nums">₦{transportTotalIncome.toLocaleString()}</td>
                  </tr>
               </tbody>
            </table>
         </div>
...
const StatCard = ({ label, value, trend, subValue, color, icon, highlight }) => (
  <div className={`bg-white border rounded-md p-6 shadow-sm flex flex-col justify-between group transition-all ${highlight ? 'border-emerald-500/20' : 'border-slate-200 hover:border-slate-300'}`}>
    <div className="flex justify-between items-start mb-4">
       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">{label}</p>
       {icon && <div className="text-slate-300">{icon}</div>}
    </div>
    <div>
       <h4 className={`text-2xl font-black tracking-tighter tabular-nums leading-none ${highlight ? 'text-emerald-600' : 'text-slate-900'}`}>{value}</h4>
       {trend && (
          <div className={`flex items-center gap-1.5 text-[10px] font-black mt-2 ${color === 'emerald' ? 'text-emerald-500' : 'text-rose-500'}`}>
            {color === 'emerald' ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
            {trend}
          </div>
       )}
       {subValue && (
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 opacity-60 flex items-center gap-2">
